...
**Frontend** (under `src/`, rendered by Vite; talks only to `/api/*`): `main.tsx` is just the entry point. `App.tsx` owns the shared filter state, all `/api` orchestration (data loads, import-job polling, maintenance actions) and the tab layout; per-tab UI lives in `views/` (MemoriesView, PatternsView, YouTubeView, TopicsView, TimelineView, CalendarView, SiteView, OrganizePanel, ExportPanel) and cross-tab pieces in `components/` (CommandBar, FilterRail — which privately owns FileBrowser and ImportProgress —, InsightCanvas, EventStream, EntityPanel, DayPanel, and the atoms in `primitives.tsx`). `types.ts` mirrors the API response shapes, `api.ts` has the defensive `readJson` helper (an empty/non-JSON body — e.g. a 404 for an endpoint a stale backend lacks — returns `null` instead of throwing "Unexpected end of JSON input"), and `format.ts` holds the pure pt-BR formatting/label/mask helpers. `src/styles.css` is hand-written (no Tailwind here). Radix UI + Recharts + TanStack Virtual are the only UI libs. Tabs: explore / **memórias** / **padrões** / **youtube** / **assuntos** / timeline / calendar / site / organize / export.
